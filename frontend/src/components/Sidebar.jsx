...
export default function Sidebar() {
  return (
    <>
      {/* 
        THE FIX: This invisible spacer holds the exact width of the sidebar 
        so it doesn't break the flex layout of your feed or profile pages.
      */}
      <div className="w-80 shrink-0 hidden md:block"></div>

      {/* 
        The actual Sidebar is now FIXED to the viewport. 
        It will never scroll, regardless of what the parent containers do.
      */}
      <aside className="w-80 bg-neutral-50 h-screen border-r border-neutral-100 flex flex-col p-8 pt-10 fixed top-0 left-0 z-50 overflow-y-auto">
        ...
        <nav className="flex flex-col gap-10">
            ...
            {navItems.map((item) => (
              <li key={item.name}>
                <Link href={item.href} className="flex items-center justify-between group">
                  <div className="flex items-center gap-4 text-neutral-700 group-hover:text-[#2196F3] transition-colors">
          ...
          <div className="flex flex-col gap-7 pt-12 border-t border-neutral-100">
              <h3 className="text-xs font-bold text-neutral-500 tracking-wider uppercase font-general-sans">Pass the J ;)</h3>
              <ul className="flex flex-col gap-7">
                {shareItems.map((item) => (
                  <li key={item.name}>
                    <Link href={item.href} className="flex items-center gap-4 text-neutral-700 hover:text-neutral-950 transition group">
                      <item.icon size={22} className="stroke-[2.25] group-hover:scale-105 transition-transform" />
                      <span className="text-[17px] font-semibold pt-0.5 tracking-tight font-satoshi">{item.name}</span>
                    </Link>
                  </li>
                ))}
